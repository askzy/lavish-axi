...
7. `lavish-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
   ...
   Delivery is a lease, not a delete: `takeFeedback` moves the batch into `session.leases` under a `delivery_id`, the CLI acks it (`POST /api/:key/ack`) only after the output has drained to stdout, and `--agent-reply` retires every lease on the session. A lease unacked for `FEEDBACK_LEASE_TTL_MS` (about a minute) is delivered again on the next poll, including `--timeout-ms 0`, and a waiting long-poll wakes for it; a waiting response carries `retry_after_ms` while a lease is outstanding. `writeState` writes a temp file and renames it over `state.json`, so a crash mid-write cannot truncate the store.
   ...
   Losing the last `/events/:key` SSE client for a session while a poll is active starts a per-session reconnect grace timer (`BROWSER_DISCONNECT_GRACE_MS`); a reconnect cancels it, while expiry releases the poll with the non-terminal `browser_disconnected` and leaves the stored session open. The disconnect result replaces only a `waiting` take, so it never consumes or retires a lease, and a `--timeout-ms 0` drain is unaffected. Poll cleanup cancels the timer once no poll is active, so a timeout, feedback, or explicit end cannot leave a stale disconnect for a later poll.
