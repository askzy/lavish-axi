...
// Upstream's #165 threads an `agent` argument through here so it can swap in a Codex-specific
// note keyed on CODEX_SANDBOX. This fork took only that commit's help factories, not the agent
// detection, so the contract stays no-arg: one shared wake-path rule for every harness.
...
// Ported from upstream #165: help text is built by a factory per call rather than frozen into a
// module-level constant. Upstream needed that to vary the poll guidance by invoking agent; this
// fork takes the shape without the agent parameter, because every later upstream commit expresses
// its help edits against these factories and hand-porting them into plain constants is a tax the
