...
export function createDesignOutput() {
  return {
    ...
    theme_usage: [
      ...
      "Never let a `base-*` surface inherit a semantic `*-content` text color. `alert-warning`, `alert-error`, `alert-info`, `alert-success`, and `bg-primary`-style containers set `color` for their whole subtree, while `bg-base-100`, `bg-base-200`, `card`, and `table` set no color at all - so a table or card nested inside a colored callout keeps the outer `*-content`, which is picked for the opposite luminance. In `dim` that puts `warning-content` (oklch 17%) on `base-200` (oklch 28%), about 1.4:1. Put `text-base-content` on every nested `base-*` surface, or build the callout with `alert-soft`, which tints the text instead of inverting it. The author cannot see this: it renders correctly on a light theme, where the same inheritance lands dark text on a light surface.",
